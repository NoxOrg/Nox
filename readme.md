...
```csharp
public class GetVendorMachineByIdSecurityValidator : AbstractValidator<GetVendorMachineByIdQuery>
{
    public GetVendorMachineByIdSecurityValidator(ILogger<GetVendorMachineByIdQuery> logger)
    {
        public GetVendorMachineByIdSecurityValidator(ILogger<GetVendorMachineByIdQuery> logger)
            // TODO Get VendorMachines that he can see.... 
        // For the Current User
        // TODO Get VendorMachine that he can see.... 

        // Do Validation The current user can only see VendorMachine with particular id
        RuleFor(query => query.keyId).Must(key => key == 1).WithMessage("No permissions to access this VendorMachine");            
    }
}
