...
public partial class StoresController : ODataController
{
    ...
    public async Task<ActionResult> CreateRefToStoreSecurityPasswords([FromRoute] System.String key, [FromRoute] System.String relatedKey)
    {
        ...
        var createdRef = await _mediator.Send(new CreateRefStoreToStoreSecurityPasswordsCommand(new StoreKeyDto(key), new StoreSecurityPasswordsKeyDto(relatedKey)));
        if (!createdRef)
        {
            return NotFound();
        }
    ...
    public async Task<ActionResult> DeleteRefToStoreOwner([FromRoute] System.String key)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
        ...
        var deletedRef = await _mediator.Send(new DeleteRefStoreToStoreOwnerCommand(new StoreKeyDto(key)));
        if (!deletedRef)
        {
            return NotFound();
        }
