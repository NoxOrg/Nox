...
public partial class StoreSecurityPasswordsController : ODataController
{
    ...
    public async Task<ActionResult> CreateRefToStore([FromRoute] System.String key, [FromRoute] System.String relatedKey)
    {
        ...
        var createdRef = await _mediator.Send(new CreateRefStoreSecurityPasswordsToStoreCommand(new StoreSecurityPasswordsKeyDto(key), new StoreKeyDto(relatedKey)));
        if (!createdRef)
        {
            return NotFound();
        }
