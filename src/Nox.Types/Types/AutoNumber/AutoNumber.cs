...
public sealed class AutoNumber : ValueObject<long, AutoNumber>
{
    ...
    public static AutoNumber From(long _, AutoNumberTypeOptions typeOptions) => throw new InvalidOperationException($"{nameof(AutoNumber)} can only be created with {nameof(FromDatabase)}.");
}