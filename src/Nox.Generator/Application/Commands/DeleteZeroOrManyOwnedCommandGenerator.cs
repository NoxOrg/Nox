...
internal class DeleteZeroOrManyOwnedCommandGenerator : ApplicationEntityDependentGeneratorBase
{
    ...
    {
        var templateName = @"Application.Commands.DeleteZeroOrManyOwnedCommand";
        var entityList = entities.ToList();
        foreach (var entity in entityList)
        {
            context.CancellationToken.ThrowIfCancellationRequested();
            foreach (var ownedRelationship in entity.OwnedRelationships.Where(r => r.Relationship == EntityRelationshipType.ZeroOrMany))
            {
