...
public abstract partial class TestEntityZeroOrManiesControllerBase : ODataController
{
    ...
    public async Task<ActionResult> UpdateRefToSecondTestEntityZeroOrManiesNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.String> referencesDto)
    {
        if (!ModelState.IsValid)
        {
        ...
        var relatedKeyDto = referencesDto.References.Select(x => new SecondTestEntityZeroOrManyKeyDto(x)).ToList();
        var updatedRef = await _mediator.Send(new UpdateRefTestEntityZeroOrManyToSecondTestEntityZeroOrManiesCommand(new TestEntityZeroOrManyKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
