...
public abstract partial class TestEntityZeroOrManyToExactlyOnesControllerBase : ODataController
{
    ...
    [EnableQuery]
    public virtual async Task<SingleResult<TestEntityZeroOrManyToExactlyOneDto>> Get([FromRoute] System.String key)
    {
    ...
    {
        if(testEntityZeroOrManyToExactlyOne is null)
        {
    ...
    public virtual async Task<ActionResult<TestEntityZeroOrManyToExactlyOneDto>> Patch([FromRoute] System.String key, [FromBody] Delta<TestEntityZeroOrManyToExactlyOnePartialUpdateDto> testEntityZeroOrManyToExactlyOne)
    {
        if(testEntityZeroOrManyToExactlyOne is null)
        {
            throw new Nox.Exceptions.BadRequestInvalidFieldException();
        ...
        var etag = Request.GetDecodedEtagHeader();
        var updatedKey = await _mediator.Send(new PartialUpdateTestEntityZeroOrManyToExactlyOneCommand(key, updatedProperties, _cultureCode, etag));
    ...
    {
        var etag = Request.GetDecodedEtagHeader();
        var result = await _mediator.Send(new DeleteTestEntityZeroOrManyToExactlyOneByIdCommand(new List<TestEntityZeroOrManyToExactlyOneKeyDto> { new TestEntityZeroOrManyToExactlyOneKeyDto(key) }, etag));
