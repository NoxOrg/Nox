...
public abstract partial class ThirdTestEntityZeroOrManiesControllerBase : ODataController
{
    ...
    public async Task<ActionResult> UpdateRefToThirdTestEntityOneOrManiesNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.String> referencesDto)
    {
        ...
        var relatedKeyDto = referencesDto.References.Select(x => new ThirdTestEntityOneOrManyKeyDto(x)).ToList();
        var updatedRef = await _mediator.Send(new UpdateRefThirdTestEntityZeroOrManyToThirdTestEntityOneOrManiesCommand(new ThirdTestEntityZeroOrManyKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
