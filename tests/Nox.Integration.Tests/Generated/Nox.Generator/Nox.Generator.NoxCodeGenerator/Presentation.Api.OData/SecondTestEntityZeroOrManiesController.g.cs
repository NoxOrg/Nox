...
public abstract partial class SecondTestEntityZeroOrManiesControllerBase : ODataController
{
    ...
    public async Task<ActionResult> UpdateRefToTestEntityZeroOrManiesNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.String> referencesDto)
    {
        ...
        var relatedKeyDto = referencesDto.References.Select(x => new TestEntityZeroOrManyKeyDto(x)).ToList();
        var updatedRef = await _mediator.Send(new UpdateRefSecondTestEntityZeroOrManyToTestEntityZeroOrManiesCommand(new SecondTestEntityZeroOrManyKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
