...
public abstract partial class TestEntityZeroOrManiesControllerBase : ODataController
{
    ...
    public virtual async Task<ActionResult<TestEntityZeroOrManyDto>> Patch([FromRoute] System.String key, [FromBody] Delta<TestEntityZeroOrManyPartialUpdateDto> testEntityZeroOrMany)
    {
        if(testEntityZeroOrMany is null)
        {
            throw new Nox.Exceptions.BadRequestInvalidFieldException();
    ...
    {
        var etag = Request.GetDecodedEtagHeader();
        var result = await _mediator.Send(new DeleteTestEntityZeroOrManyByIdCommand(new List<TestEntityZeroOrManyKeyDto> { new TestEntityZeroOrManyKeyDto(key) }, etag));
