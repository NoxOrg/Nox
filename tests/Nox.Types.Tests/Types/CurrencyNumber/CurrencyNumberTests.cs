...
public class CurrencyNumberTests
{
    ...
    {
        var action = () => CurrencyNumber.From(999);

        action.Should().Throw<TypeValidationException>()
            .And.Errors.Should().BeEquivalentTo(new[] { new ValidationFailure("Value", "Could not create a Nox CurrencyNumber type with unsupported value '999'.") });
    }
