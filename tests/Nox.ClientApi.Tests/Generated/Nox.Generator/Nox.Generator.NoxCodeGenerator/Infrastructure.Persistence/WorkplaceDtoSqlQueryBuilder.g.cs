...
public class WorkplaceDtoSqlQueryBuilder : IEntityDtoSqlQueryBuilder
{
	...
	private static Query WorkplaceQuery()
	{
		return new Query("Workplaces")
			...
			.ForSqlServer(q => q.SelectRaw("COALESCE([WorkplacesLocalized].[Description], CASE WHEN [Workplaces].[Description] IS NULL THEN N'' ELSE N'[' + [Workplaces].[Description] + N']' END) AS [Description]"))
			.ForPostgreSql(q => q.SelectRaw("COALESCE(\"WorkplacesLocalized\".\"Description\",CASE WHEN \"Workplaces\".\"Description\" IS NULL THEN '' ELSE '##OPEN##' || \"Workplaces\".\"Description\" || '##CLOSE##' END) AS \"Description\""))
			.ForSqlite(q => q.SelectRaw("COALESCE(\"WorkplacesLocalized\".\"Description\",CASE WHEN \"Workplaces\".\"Description\" IS NULL THEN '' ELSE '##OPEN##' || \"Workplaces\".\"Description\" || '##CLOSE##' END) AS \"Description\""))
