...
public partial class StoreOwnersController : ODataController
{
    ...
    public async Task<ActionResult> CreateRefToStores([FromRoute] System.String key, [FromRoute] System.UInt32 relatedKey)
    {
        ...
        var createdRef = await _mediator.Send(new CreateRefStoreOwnerToStoreCommand(new StoreOwnerKeyDto(key), new StoreKeyDto(relatedKey)));
        if (!createdRef)
        {
            return NotFound();
        }
    ...
    public async Task<ActionResult> DeleteRefToStores([FromRoute] System.String key, [FromRoute] System.UInt32 relatedKey)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
