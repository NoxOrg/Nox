using Microsoft.EntityFrameworkCore;
using Nox.Abstractions;
using Nox.Solution;
using Nox.Types.EntityFramework.Abstractions;
using Nox.Types.EntityFramework.Enums;

namespace Nox.Localization.Tests.Common;

public class TestDbContext: DbContext
{
    private readonly NoxSolution? _noxSolution;
    private readonly INoxDatabaseProvider? _dbProvider;
    private readonly INoxClientAssemblyProvider? _clientAssemblyProvider;
    
    public TestDbContext(
            DbContextOptions<TestDbContext> options,
            NoxSolution noxSolution,
            IEnumerable<INoxDatabaseProvider> databaseProviders,
            INoxClientAssemblyProvider clientAssemblyProvider
        ) : base(options)
        {
            _noxSolution = noxSolution;
            _dbProvider = databaseProviders.Single(p => p.StoreType == NoxDataStoreType.EntityStore);
            _clientAssemblyProvider = clientAssemblyProvider;
        }


    public DbSet<TestEntity> TestEntities { get; set; } = null!;
    
    public DbSet<Translation> Translations { get; set; } = null!;


    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        base.OnConfiguring(optionsBuilder);
        if (_noxSolution is { Infrastructure.Persistence.DatabaseServer: not null })
        {
            _dbProvider!.ConfigureDbContext(optionsBuilder, "LocalizationTests", _noxSolution.Infrastructure!.Persistence.DatabaseServer); 
        }
    }

    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        base.OnModelCreating(modelBuilder);
 
        if (_noxSolution is { Domain: not null })
        {
            var codeGeneratorState = new NoxSolutionCodeGeneratorState(_noxSolution, _clientAssemblyProvider!.ClientAssembly);
            foreach (var entity in _noxSolution.Domain.Entities)
            {
                Console.WriteLine($"LocalizationTests Configure database for Entity {entity.Name}");
                var type = codeGeneratorState.GetEntityType(entity.Name);
                if (type != null)
                {
                    ((INoxDatabaseConfigurator)_dbProvider!).ConfigureEntity(codeGeneratorState, modelBuilder.Entity(type), entity, _noxSolution.GetRelationshipsToCreate(codeGeneratorState.GetEntityType, entity));
                }
            }
        }
    }
}