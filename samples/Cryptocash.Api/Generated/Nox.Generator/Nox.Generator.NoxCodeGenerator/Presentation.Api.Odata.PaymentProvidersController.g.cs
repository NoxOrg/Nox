...
public partial class PaymentProvidersController : ODataController
{
    ...
    public async Task<ActionResult> CreateRefToCustomerPaymentDetails([FromRoute] System.Int64 key, [FromRoute] System.Int64 relatedKey)
    {
        ...
        var createdRef = await _mediator.Send(new CreateRefPaymentProviderToCustomerPaymentDetailsCommand(new PaymentProviderKeyDto(key), new CustomerPaymentDetailsKeyDto(relatedKey)));
        if (!createdRef)
        {
            return NotFound();
        }
