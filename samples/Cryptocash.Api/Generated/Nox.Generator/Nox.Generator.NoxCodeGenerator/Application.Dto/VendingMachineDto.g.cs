...
public abstract class VendingMachineDtoBase : EntityDtoBase
{
    ...
    public virtual IReadOnlyDictionary<string, IEnumerable<string>> Validate()
    {
        ...
        if (this.MacAddress is not null)
            CollectValidationExceptions("MacAddress", () => VendingMachineMetadata.CreateMacAddress(this.MacAddress.NonNullValue<System.String>()), result);
        else
            result.Add("MacAddress", new [] { "MacAddress is Required." });
    
        if (this.PublicIp is not null)
            CollectValidationExceptions("PublicIp", () => VendingMachineMetadata.CreatePublicIp(this.PublicIp.NonNullValue<System.String>()), result);
        else
            result.Add("PublicIp", new [] { "PublicIp is Required." });
    
        if (this.GeoLocation is not null)
            CollectValidationExceptions("GeoLocation", () => VendingMachineMetadata.CreateGeoLocation(this.GeoLocation.NonNullValue<LatLongDto>()), result);
        else
            result.Add("GeoLocation", new [] { "GeoLocation is Required." });
    
        if (this.StreetAddress is not null)
            CollectValidationExceptions("StreetAddress", () => VendingMachineMetadata.CreateStreetAddress(this.StreetAddress.NonNullValue<StreetAddressDto>()), result);
        else
            result.Add("StreetAddress", new [] { "StreetAddress is Required." });
    
        if (this.SerialNumber is not null)
            CollectValidationExceptions("SerialNumber", () => VendingMachineMetadata.CreateSerialNumber(this.SerialNumber.NonNullValue<System.String>()), result);
        else
            result.Add("SerialNumber", new [] { "SerialNumber is Required." });
    
        if (this.InstallationFootPrint is not null)
            CollectValidationExceptions("InstallationFootPrint", () => VendingMachineMetadata.CreateInstallationFootPrint(this.InstallationFootPrint.NonNullValue<System.Decimal>()), result);
        if (this.RentPerSquareMetre is not null)
            CollectValidationExceptions("RentPerSquareMetre", () => VendingMachineMetadata.CreateRentPerSquareMetre(this.RentPerSquareMetre.NonNullValue<MoneyDto>()), result);
