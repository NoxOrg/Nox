...
public partial class CustomerPaymentDetailsController : ODataController
{
    ...
    public async Task<ActionResult> CreateRefToPaymentProvider([FromRoute] System.Int64 key, [FromRoute] System.Int64 relatedKey)
    {
        ...
        var createdRef = await _mediator.Send(new CreateRefCustomerPaymentDetailsToPaymentProviderCommand(new CustomerPaymentDetailsKeyDto(key), new PaymentProviderKeyDto(relatedKey)));
        if (!createdRef)
        {
            return NotFound();
        }
