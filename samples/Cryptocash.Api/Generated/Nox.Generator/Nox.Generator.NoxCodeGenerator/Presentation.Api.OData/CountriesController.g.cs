...
public abstract partial class CountriesControllerBase : ODataController
{
    ...
    public async Task<ActionResult> UpdateRefToCommissionsNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.Int64> referencesDto)
    {
        ...
        var relatedKeyDto = referencesDto.References.Select(x => new CommissionKeyDto(x)).ToList();
        var updatedRef = await _mediator.Send(new UpdateRefCountryToCommissionsCommand(new CountryKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
    ...
    public async Task<ActionResult> UpdateRefToVendingMachinesNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.Guid> referencesDto)
    {
        ...
        var updatedRef = await _mediator.Send(new UpdateRefCountryToVendingMachinesCommand(new CountryKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
    ...
    public async Task<ActionResult> UpdateRefToCustomersNonConventional([FromRoute] System.String key, [FromBody] ReferencesDto<System.Int64> referencesDto)
    {
        ...
        var relatedKeyDto = referencesDto.References.Select(x => new CustomerKeyDto(x)).ToList();
        var updatedRef = await _mediator.Send(new UpdateRefCountryToCustomersCommand(new CountryKeyDto(key), relatedKeyDto));
        if (!updatedRef)
        {
            return NotFound();
        }
