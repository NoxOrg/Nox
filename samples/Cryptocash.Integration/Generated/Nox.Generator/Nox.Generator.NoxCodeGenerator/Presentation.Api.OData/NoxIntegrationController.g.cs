...
public class NoxIntegrationController : Controller
{
    ...
    [SwaggerOperation(
        Description = "Execute integration ProcToTable",
        OperationId = "ExecuteProcToTable"
    )]
    [HttpPost("[action]")]
    public async Task<ActionResult> ExecuteProcToTable()
    {
        try
        {
            await _integrationContext.ExecuteIntegrationAsync("ProcToTable");
            return Ok();
        }
        catch (Exception ex)
        {
