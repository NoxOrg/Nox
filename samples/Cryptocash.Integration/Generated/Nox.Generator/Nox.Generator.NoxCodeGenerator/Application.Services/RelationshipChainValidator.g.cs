...
#region Constructor
    public  RelationshipChainValidatorBase(IRepository repository)
    {
        ...
        _entityContextPerEntityName = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "CountryProcToTables", (Repository.Query<CryptocashIntegration.Domain.CountryProcToTable>(), "CountryId") }
        };
    ...
    private bool TryParseKey(string entityName, string key, out Nox.Types.INoxType parsedKey)
    {
        ...
        if (entityName.Equals("CountryProcToTables", StringComparison.OrdinalIgnoreCase))
        {
            if (!System.Int32.TryParse(key, out var value)) return false;
            parsedKey = CountryProcToTableMetadata.CreateCountryId(value);
            return true;
        }
        return false;
    }
