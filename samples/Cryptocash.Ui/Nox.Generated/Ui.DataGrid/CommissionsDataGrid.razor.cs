...
{
    private List<CommissionModel> CommissionsData = new(); 
    ...
    public async Task<TableData<CommissionModel>> ServerReload(TableState State, CancellationToken cancellationToken)
    {
        ...
        return new TableData<CommissionModel>() { TotalItems = Convert.ToInt32(CommissionEntityData?.EntityTotal), Items = PagedData };
    }
    ...
    private async Task GetApiEntityData()
    {
            ...
            || !String.Equals(PreviousAPiQuery, CurrentApiQuery, StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (IsPagingPopulated)
            {
                CurrentApiUiService!.Paging!.SetEntityTotal(CommissionEntityData?.EntityTotal);
            }

            PagedData = CommissionEntityData?.EntityList;
        }
    }
    ...
    public async Task Search()
    {
        ...
        if (CommissionDataGridTable != null)
        {
            await CommissionDataGridTable!.ReloadServerData();
        }
...
#pragma warning disable BL0005 // Component parameter should not be set outside of its component. 
                CommissionDataGridTable!.CurrentPage = 0; //only reliable way to reset page on reorder
