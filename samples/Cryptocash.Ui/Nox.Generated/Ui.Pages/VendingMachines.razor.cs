...
public partial class VendingMachines : ComponentBase
{

    public ApiUiService CurrentApiUiService { get; set; } = new();

    private VendingMachinesDataGrid? referencedDataGrid;



    protected override void OnInitialized()
    {
        CurrentApiUiService = VendingMachinesService.IntialiseApiUiService();
    }
...
#region ADD
    ...
    private async Task AddVendingMachineSubmit()
    {
        showAddDialog = false;        
        await Search();
    }
...
#region EDIT
    ...
    [Parameter]
    public VendingMachineModel? SelectedEntity { get; set; }   

    private void ShowEditDialog(VendingMachineModel? currentSelected)
    {
    ...
    private async Task EditVendingMachineSubmit()
    {
        showEditDialog = false;    
        SelectedEntity = null;
        await Search();        
...
#region DELETE
    ...
    [Parameter]
    public VendingMachineModel? DeleteEntity { get; set; }   
    ...
    private void ShowDeleteConfirmation(VendingMachineModel? currentSelected)
    {
    ...
    private async Task DeleteVendingMachineSubmit()
    {
        try
        {
            ...
            {
                await VendingMachinesService.DeleteAsync(DeleteEntity);
            }
            else
            {
                HasDeleteError = true;
